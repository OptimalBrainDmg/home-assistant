...
- **`epaper_weather/` does not use MQTT** — it fetches data via the HA REST API (`/api/states` and `/api/services/weather/get_forecasts`) using a long-lived access token. There is no broker connection.
