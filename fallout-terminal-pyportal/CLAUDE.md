...
- **Adafruit SleepyDog Library** by Adafruit *(not installed — must add; hardware watchdog)*
...
- **Touch mapping**: `touchToScrX/Y()` maps raw resistive-touch ADC coordinates to screen pixels for `setRotation(3)` (landscape, USB at bottom — 180° mount). Both functions use `constrain()` to clamp out-of-range readings. Constants are empirically calibrated from serial output — to recalibrate, monitor serial while pressing known screen corners and back-calculate `TOUCH_X/Y_MIN/MAX` from the printed `raw=p.x,p.y` values. **Axis note**: `touchToScrX` uses `p.y` (not `p.x`) because the touchscreen axes are swapped relative to the display in landscape rotation; both `map()` directions are low→high (opposite of rotation 1) because rotation 3 mirrors the display 180°.
...
- **Hardware watchdog**: `Watchdog.enable(16000)` (Adafruit_SleepyDog, SAMD51's hardware max) is armed at the end of `setup()`, after the fatal SD-halt path and the initial boot chime — arming earlier would let the watchdog interrupt the intentional SD-failure halt loop or a slow first WAV playback. `petWatchdog()` is called once per `loop()` iteration and is also registered via `WiFi.setFeedWatchdogFunc()`, so WiFiNINA's own internal connection-wait loops (`WiFi.begin()` can block up to 50 s) keep petting it instead of tripping a false reset. If any single call blocks longer than 16 s without going through one of those two paths — e.g. a wedged low-level WiFiNINA SPI transfer, which is the suspected cause of hard lockups that previously required a power cycle — the watchdog resets the SAMD51. Because `SpiDrv::begin()` (in the `Arduino_SpiNINA` library WiFiNINA depends on) toggles the NINA co-processor's reset pin (`NINA_RESETN`) on its first call after any boot — the guard flag is a RAM static, cleared by any reset — a watchdog-triggered reset also resets the ESP32 co-processor, not just the SAMD51. `resetCauseName()` reads `RSTC->RCAUSE` and logs it over serial at boot so a recovered hard lock (cause `WDT`) can be told apart from a normal power-on; this is not currently published to MQTT. **Unverified**: this recovers a hard lock once it happens, it does not address the root cause, which has not been confirmed via serial capture of an actual lockup.
